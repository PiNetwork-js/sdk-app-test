...
import { Pi, AuthResult } from '@pinetwork-js/sdk'
...
export default class Demo extends React.Component<Props, DemoState> {
  pinetwork = Pi;
  ...
  loadUser = async () => {
    try {
      const result: AuthResult = await this.pinetwork.authenticate([], this.onIncompletePaymentFound);
  ...
  onIncompletePaymentFound = (payment: APIPayment) => {
    ...
    this.mockDevCompletion(payment.identifier, payment.transaction && payment.transaction.txid);
  }
  ...
  mockDevApproval = async (paymentId: string) => {
    return axios.create({
      ...
      timeout: 20000,
    }).post(`/v2/payments/${paymentId}/approve`, {}, { headers: { 'Authorization': 'Key pinetwork-js-sdk-test' } })
    .then(payment => console.log("mockDevApproval response:", payment));
  }

  mockDevCompletion = async (paymentId: string, txid?: string | null) => {
    if (!txid) throw new Error("can't mockDevCompletion without a txid");

    return axios.create({
      baseURL: 'https://socialchain.app',
      timeout: 20000,
    }).post(`/v2/payments/${paymentId}/complete`, { txid }, { headers: { 'Authorization': 'Key pinetwork-js-sdk-test' } })
  ...
  requestTransfer = async () => {
    ...
    try {
      ...
      await this.pinetwork.createPayment(paymentData, callbacks);
    } catch(err) {
  ...
  share = () => {
    this.pinetwork.openShareDialog('Check out the demo app!', "Join the Pi Network developer program and create your own app on Pi Network.");
  }
    ...
    return (
      <div style={{flexBasis: '48%', padding: '8px'}}>
        ...
        <div style={{display: 'flex', justifyContent: 'center', alignItems: 'center', width: '100%'}}>
          <div style={{padding: '8px 16px', backgroundColor: 'lightblue'}} onClick={this.requestTransfer}>
            Send 1 Test-π
          </div>
        </div>
      </div>
    ...
    return (
      <div>
        <div style={{padding: '16px'}}>
          ...
          <hr />
          <div onClick={this.share} style={{padding: '8px 16px', backgroundColor: 'lightblue'}}>
